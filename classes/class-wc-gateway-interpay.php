...
/**
 * InterPay Interac® Online Redirected Payment Gateway.
 *
 * Provides an off site Secured Payment Gateway to Interac® Online by InterPay.
 *
 * @class       WC_Gateway_InterPay
...
/**
 * WC_Gateway_InterPay Class.
 */
class WC_Gateway_InterPay extends WC_Payment_Gateway {
        ...
        */
        public function __construct() {
                ...
                $this->method_description = sprintf( __( 'InterPay sends customers to a Secure Interac® Online Website in order to complete payment. InterPay requires callbacks to update order statuses after payment. Check the %ssystem status%s page for more details.', 'woocommerce' ), '<a href="' . admin_url( 'admin.php?page=wc-status' ) . '">', '</a>' );
        ...
        */
        public function is_valid_for_use() {
                return in_array( get_woocommerce_currency(), apply_filters( 'woocommerce_interpay_supported_currencies', array( 'CAD' ) ) );
        }
        ...
        */
        public function admin_options() {
                if ( in_array( get_woocommerce_currency(), $this->available_currencies ) ) {
                        ?>
                        <h3><?php _e( 'InterPay\'s Interac® Online Payment Gateway', 'woocommerce' ); ?></h3>
                        <table class="form-table">
                                ...
                                <div class="inline error"><p><strong><?php _e( 'Gateway Disabled', 'woocommerce-gateway-interpay' ); ?></strong> <?php echo sprintf( __( 'Choose Canadian Dollars as your store currency in %1$sPricing Options%2$s to enable the InterPay Interac® Online Payment Gateway.', 'woocommerce-gateway-interpay' ), '<a href="' . esc_url( admin_url( 'admin.php?page=wc-settings&tab=general' ) ) . '">', '</a>' ); ?></p></div>
                                <?php
                }
        }
...
error_log( get_bloginfo( 'url' ),0);
                $this->form_fields = array(
                        ...
                        'description' => array(
                                ...
                                'termid' => array(
                                'title'       => __( 'InterPay TERMID', 'woocommerce' ),
                                'type'        => 'text',
                                'description' => __( 'Please enter your InterPay TERMID. Should be 8 Characters long. Required to process payments!', 'woocommerce' ),
                                'desc_tip'    => true,
                        ),
                        'pass' => array(
                                'title'       => __( 'InterPay PASS', 'woocommerce' ),
                                'type'        => 'text',
                                'description' => __( 'Please enter your InterPay Password. Required to process payments!', 'woocommerce' ),
                                'desc_tip'    => true,
                        ),
                        'testmode' => array(
                                'title'       => __( 'InterPay\'s Interac® Online Sandbox', 'woocommerce' ),
                                'type'        => 'checkbox',
                                'label'       => __( 'Enable InterPay\'s Interac® Online sandbox', 'woocommerce' ),
                                'default'     => 'no',
                                'description' =>  __( 'InterPay sandbox can be used to test payments.', 'woocommerce' ),
                        ),
					
						'payment_success_status' => array(
                                'title'       => __( 'InterPay\'s Interac® Online Successful Order Status', 'woocommerce' ),
                                'type'        => 'select',
							    'options' => array(
                                ...
                                'label'       => __( 'Select InterPay\'s Interac® Online successful order status', 'woocommerce' ),
                                'default'     => 'on-hold',
                                'description' =>  __( 'Interpay\'s Interac® Online plugin changes orders to this status upon successful order completion.', 'woocommerce' ),
                        ),
					
					    array(
                                'title' => __( 'InterPay\'s Return URL', 'woocommerce' ),
                                'type' => 'title',
        ...
        */
        function checkOrderAfter($order_id) {
                ...
                if ( 'pending' == $_Customer_Order->status ) {
                        ...
                        if ( $valid = true ) {
                                ...
                                if ( !is_wp_error( $_GetResult_Response ) ) {
                                        ...
                                        if ( $out['TEXT'] == 'APPROVED' ) {
												...
												error_log($_Interpay_Gateway->get_success_status());
											
                                                $_Customer_Order->update_status($_Interpay_Gateway->get_success_status(), 'Order Successful! From TriForce Interac Plugin');
                                                ...
                                                // Remove cart
                                                WC()->cart->empty_cart();
                                                error_log('checkOrderAfter(): Success Redirect: '.$_Interpay_Gateway->get_return_url( $_Customer_Order ),0);
                                                if ( wp_redirect( $_Interpay_Gateway->get_return_url( $_Customer_Order ) ) ) {
                                                        exit;
                                                }
						...
						error_log('checkOrderAfter(): Failure Redirect: '.$_Interpay_Gateway->get_return_url( $_Customer_Order ),0);
        ...
        */
        function process_payment( $order_id ) {
                ...
                $_Interpay_Response = wp_remote_get( $start_url, array( 'timeout' => 120, 'httpversion' => '1.1', 'sslverify' => true ));


                if ( ! is_wp_error( $_Interpay_Response ) ) {
                        update_post_meta( $order_id, 'Received Data 1', $_Interpay_Response['body'] );
						
						if ( $this->debugmode ) {
							error_log($_Interpay_Response['body']);
						}
                        
						$interpay_url = $this->extractQuery( str_replace("%38","&",$_Interpay_Response['body']) , 'URL').'&SecureTYPE=GET';
                        $this->secure_id = $this->extractQuery($_Interpay_Response['body'], 'SECUREID');
                        ...
                        if ( $this->debugmode ) {
							error_log( $_Interpay_Response->get_error_message() ,0);
						}
                }

                if ( is_wp_error( $_Interpay_Response ) || ( strlen( $interpay_url ) < 15 ) ) {
